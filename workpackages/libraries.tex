...
\WPref{instrumentation}.  This WP focuses on advanced libraries
selected according to the following criteria: relevance, coverage and
maturity.  As a result we selected the following libraries: the
Archive of Formal Proofs, Isabelle's revised Analysis and Probability
library, GeoCoq, Flyspeck, and CakeML. 
\end{wpdescription}


\begin{tasklist}
%\begin{task}[id=mathcomp,title=MathComp,shorttitle=MathComp]
%\ednote{Sophia, Saclay (Gonthier), Paris}
%\end{task}

%\begin{task}[id=milc,title=Revised Coq Analysis Library,shorttitle=Coq Ana.]
%\ednote{Saclay (Boldo), Paris, Sophia}
%\end{task}

%\begin{task}[id=mizar,title=The Mizar library,shorttitle=Mizar]
...
conjecture, based on an original proof of Thomas {Hales}, and formalised
largely in {HOL Light}.% \url{https://github.com/flyspeck/flyspeck}.
Some of these results are not formalised in any other system, motivating the
...
%\begin{task}[id=unimath,title=The UniMath library,shorttitle=UniMath]
%\ednote{Birmingham (Ahrens)}
%\end{task}

%\begin{task}[id=pvs,title=The NASA PVS library,shorttitle=NASA PVS]
%\end{task}
%\begin{task}[id=sel4,title=The seL4 library,shorttitle=seL4]
%\end{task}

%\begin{task}[id=compcert,title=The CompCert library,shorttitle=CompCert]
%\end{task}
\end{tasklist}

\begin{wpdelivs}
  \begin{wpdeliv}[due=36,id=exportafp,dissem=PU,nature=DEM,lead=Tum,task=afp]{Scalable export of proof terms for major parts of Isabelle/AFP}\end{wpdeliv}
  
  \begin{wpdeliv}[due=36,id=exportanalysis,dissem=PU,nature=DEM,lead=Tum,task=isaAnalysisProb]{Export of Isabelle's extended analysis and probability theory library}\end{wpdeliv}
  
  \begin{wpdeliv}[due=18,id=geocoq-import,dissem=PU,nature=OTH,lead=Str,task=geocoq]{Export of most of GeoCoq library}\end{wpdeliv}
  
  \begin{wpdeliv}[due=36,id=flyspec,dissem=PU,nature=OTH,lead=Inr,task=flyspeck]{Export some reusable parts of the multivariate analysis library from the Flyspeck library}\end{wpdeliv}

  \begin{wpdeliv}[due=26,id=cakeml,dissem=PU,nature=DEM,lead=Cha,task=cakeml]{Export parts of the CakeML library (as a minimum: the source semantics)}\end{wpdeliv}
\end{wpdelivs}
\end{workpackage}
