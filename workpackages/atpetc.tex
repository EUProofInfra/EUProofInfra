\begin{workpackage}[id=atpetc,type=RTD,wphases=1-48,
...
model checkers is increasingly recognised.  While for the
...
If a standardised proof format appears for some kind of tools, the
...
This work package has the end-to-end objective to build a powerful automated
...
%
    ...
    provers from \taskref{atpetc}{instrumenting}.  This task depends on
    \taskref{atpetc}{instrumenting}, but can be partly executed in
    parallel.
...
%
    ...
    to translate Logipedia statements into the standardised input format for
...
%
    ...
    \begin{compactitem}
    \item the proof obligations rewrite engine and the translation of the proof obligations to the ATPs
...
% \begin{tasklist}
%   \begin{task}[id=tools,title=Automatic Tools Exporting Proofs,shorttitle=Export]
...
%   \begin{task}[id=challenges,title=Logipedia as a Source of Challenges for Automatic Reasoners,shorttitle=Source]
...
%     %% Josef Urban: TPTP
%   \end{task}
%   \begin{task}[id=commang,title=Cooperation of Reasoners via Dedukti/Logipedia,shorttitle=Cooperation]
...
%   \end{task}
%   \begin{task}[id=database,title=A Database of Theorems for Automatic Solvers,shorttitle=Database]
...
\begin{wpdelivs}
  \begin{wpdeliv}[due=6,id=ATPtrace1,dissem=PU,nature=DEM,lead=Lie,task=instrumenting]{Prototypes of Trace producing Automatic Theorem Prover and Satisfiability Modulo Theories solver}\end{wpdeliv}

  \begin{wpdeliv}[due=18,id=Trace2Dedukti1,dissem=PU,nature=DEM,lead=Imt,task=tracetodedukti]{Prototype of a translator for some traces to Dedukti}\end{wpdeliv}

  \begin{wpdeliv}[due=24,id=Dedukti2ATP1,dissem=PU,nature=DEM,lead=Sac,task=deduktitoatp]{Prototype of a translator from Dedukti statements into TPTP and SMT-LIB, application to simple libraries}\end{wpdeliv}

  \begin{wpdeliv}[due=36,id=ReadinessCode,dissem=PU,nature=DEM,lead=Sac,task=readiness]{Tools based on ATPs to increase Logipedia Readiness}\end{wpdeliv}

  \begin{wpdeliv}[due=40,id=ATPtrace2,dissem=PU,nature=OTHER,lead=Lie,task=instrumenting]{Trace producing Automatic Theorem Prover, Satisfiability Modulo Theories solvers, and Coherent Logic Solver}\end{wpdeliv}

  \begin{wpdeliv}[due=48,id=Trace2Dedukti2,dissem=PU,nature=OTHER,lead=Imt,task=tracetodedukti]{Translate most traces to Dedukti}\end{wpdeliv}

  \begin{wpdeliv}[due=36,id=Dedukti2ATP2,dissem=PU,nature=OTHER,lead=Sac,task=deduktitoatp]{Translator from expressive Dedukti statements into TPTP and SMT-LIB, application to large libraries}\end{wpdeliv}

  \begin{wpdeliv}[due=48,id=ReadinessReport,dissem=PU,nature=R,lead=Inn,task=readiness]{Report on Proof Automation in Logipedia}\end{wpdeliv}

  \begin{wpdeliv}[due=48,id=Why3,dissem=PU,nature=DEM,lead=Cea,task=cooperation]{A posteriori verification of non trivial C programs in the Frama-C-WP platform}\end{wpdeliv}
