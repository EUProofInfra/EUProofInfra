...
\begin{tasklist}
  ...
  % François Bobot (Why3)
    ...
    \item Resources: 15 p.m engineer (Oca) + 36 p.m phd (Lie+Stu) + 3 p.m + 10 p.m engineer (Cle)
      permanent (Bel)
    \end{compactitem}
...
\end{task}
  ...
  \begin{task}[id=deduktitoatp,
    ...
    some automatic tools, namely TPTP and SMT-LIB
...
% \end{itemize}
  ...
  \begin{task}[id=reconstruction,
      title=ATPs for Logipedia,
      lead=Inn
    ]
    ...
    \item Milestones: TODO
    \item Depends on T3
    \item Resources: 6 p.m post-doc (Inn) + 1 p.m permanent (Sac)
    \end{compactitem}
...
\item Goal: TODO
\item Milestones: TODO
\item Resources: 3 p.m permanent (Bel) + 3 p.m permanent (Sac)
\end{compactitem}
