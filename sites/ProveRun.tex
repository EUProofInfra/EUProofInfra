\begin{sitedescription}{Pro}
  % To editor: don't know if it is OK to define a command here,
  % I assume the risk of another command called \provenrun
  % already existing is pretty slim...
...
\ednote{specify the main tasks and reference the respective work
  packages}

\begin{compactitem}
\item \WPref{???}, task \taskref{???} (WP2, Task ..?): Translate models and proofs from ProvenTools to Dedukti (Lescuyer)
\item \WPref{dissemination}, task \taskref{dissemination}{certif-club}: ... (Dufay)
\end{compactitem}
