...
Concretely, we plan to tackle the following research challenges:
\begin{description}
...
The work on instrumentation is a central part of this project and thus
interacts with several of the other work packages. Specifically:
\begin{itemize}

  \item WP2: In order to implement the translation from a system $X$ to
  Dedukti or to make the translation more efficient in terms of proof
  size, we may need to modify or improve the encoding of system $X$ in
  Dedukti.

  \item WP3: The instrumentation of a system $X$ is necessary before
  it can be attempted to export libraries of system $X$ to
  Dedukti. Testing the translation on large libraries might also point
  us towards possible ways to improve the translator.

  \item To instrument systems that do not store complete proof terms,
  it will be necessary to use automated theorem provers to fill in the
  missing details
