...
other logical frameworks such as $\lambda$-Prolog
...
associated proof library in Logipedia, we introduce a metric: {\em the
  Logipedia integration levels} (Figure \ref{lil}) that counts six
levels.
...
first half of the array (Matita, HOL Light, FoCaLize, Coq, Agda,
Atelier B, ProB, Isabelle/HOL, HOL4, TSTP, and Smart) for which we
have preliminary results and that we plan to bring to a very high
level of integration. Those in the second half (Minlog, PVS, Abella,
Mizar, Smart, TLA$^+$, SMT-Lib, Why3, and LFSC) with which we are
...
respectivement in EventB and B method.  The development process for
...
used, depending on the the tradeoff between computation time and space
for storage.
...
\item In the context of the BWare project, an expression of the set
  ...
  by the automatic prover Zenon modulo which features a backend to
...
step of the trace is transformed into an independent subproblem; each
of these subproblems is given to a prover that can output Dedukti
proofs; proofs of the subproblems are then combined to produce a
global proof of the original problem.  Since subproblems correspond to
...
subproblems; it is only required that the prover can output a Dedukti
...
\paragraph*{Translate Dedukti statements into automatic theorem provers
  inputs.}
Automatic theorem provers are mostly based on (parts of) predciate
...
following criteria:
\begin{itemize}
\item Relevance: the libraries should support core areas of matematics
...
Finally, after discussing the concetps related to the networking activities
needed to collect the content of Logipedia and the infrastructure developmnent
nedded to make it accessible, let us turn to joint research projects. 
...
\paragraph*{Abella.}
Abella\footnote{\url{http://abella-prover.org}}~\cite{baelde:abella}
is an interactive theorem prover that is particularly well suited to
formalizing the meta-theory of formalisms that can be specified by a
system of inference rules, including type systems, proof systems or
process calculi. Formalizations that have been carried out in Abella
include representations of different variants of the $\pi$-calculus
with the theory of bisimulation (including open bisimulation and
bisimulation up to equivalence relations), the meta-theory of the
$\lambda$-calculus with proofs of (weak and strong) normalization and
standardization, an encoding of PCF with a formalization of Howe's
techniques of establishing applicative bisimilarity and a verified
compiler to an intermediate representation in predciate logic using
step-indexed logical relations, as well as one of the first proposed
solutions to the POPLMark challenge.

At its core, Abella is based on constructive predicate logic specified using
...
this purpose, predicate logic is extended by considering both equality and the
...
The fixed point connectives allow users to define inductive and
co-inductive logical relations, without the restriction to Horn
clauses (as in Prolog). For example, constructive logic and
constructive Simple type theory thenselves, can be defined
inductively as an object logic. In this way, one can reason about the
relational specification of an object logic. Abella makes use of terms
of the $\lambda$-calculus, together with their equational theory, to
represent the syntactic constructs of the encoded systems. This
representation is known as $\lambda$-tree syntax, a form of
higher-order abstract syntax where $\lambda$-terms are considered
modulo $\alpha\beta\eta$-equivalence, and it yields a declarative,
high-level, and versatile approach for representing binding constructs
of encoded formalisms, such as name restriction in process calculi.
...
Abella and Minlog have strong support for coinduction and corecursion
...
proof search, and PVS emphasizes predicate subtyping, which is also
...
subtyping, whereas Mizar and TLA$^+$ are based on classical set theories. The
