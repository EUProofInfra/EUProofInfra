...
limitations of predicate logic and to propose more powerful logical frameworks.
The most prominent limitations of predicate logic are
...
CompCert C compiler and complex mathematical proofs such as those of
the Four Color theorem and the Odd Order theorem. In 2013
...
\paragraph*{Systems based on set theory and predicate logic (Atelier B,  Rodin, ProB).}
...
addition, they have the other advantages of being modular, easily extensible, and
...
necessary to use a wide variety of solvers in a
portfolio manner. For instance, the Why3 tool features the ability to send problems
to lots of provers in a uniform way. However, with so many tools
written by different teams, the meaning of the same concept
in the different tools could be different. This can lead to errors in
...
point numbers whose exact meaning is less clear that mathematical
integers. This leads to more chances for two tools to interpret
the same concept differently. Finally, industrial users sometimes
need to add a kind of reason or a simplification that is specific to their particular
...
Dedukti may increase this size by one or two orders of magnitude as
the Dedukti proof format is of a lower level than the high level
languages used by humans in interactive proof assistants for defining
objects or proving properties.

Moreover, these mathematical objects can have many dependencies
between each other: a proof of a theorem usually relies on the
definition of various objects and other proofs as well
(lemmas). Hence, verifying the correctness of a proof requires to
check the correctness of other proofs, and translating a proof
requires the translation of many other definitions and proofs.
...
\begin{figure}[h]
    \begin{center}
      ...
      \end{tabular}
      \caption{Examples of big proof libraries\label{fig-stats}}
    \end{center}
\end{figure}

\subsubsection*{Continuous integration and deployment}

\subsubsection*{Online FAIR access}
...
Internet. Even in the industry, only the parts that are very specific
...
accessible from the Internet. This will ensure findability,
...
We will use several metrics to measure the success of the platform:
\begin{compactitem}
...
Having discussed the concepts related to the networking activities required for
collecting the content of Logipedia and to the development of the infrastructure
needed to make it accessible, we will describe joint research projects.

So far, we have considered systems that were already at LIL 1, that is,
systems that implement a theory that we know how to express in Dedukti.
Other relevant systems are still at level 0, that is, we
do not yet know how to express them. Part of our project is to work on such
systems, in order to understand how to express their theory in Dedukti.
...
Matching logic~\cite{rosu:matching} (ML) was designed as a unifying foundational
logic for specifying and verifying programming languages. Extending first-order
modal logic by features such as many-sorted universes and least fixpoints, it
designates a family of logics that underlie a framework for describing and
reasoning about both static structures and dynamic features by means of patterns
and pattern matching. A language definition in ML presents both the operational
and the axiomatic aspects in a uniform way, with the semantics of the language
expressed as a set of rewrite rules, which are special patterns in ML. A
...
ML serves as the foundation for the K
framework\footnote{\url{https://github.com/kframework/k}} that allows the
designer of a programming languages to specify its formal semantics and to
derive analysis tools for the language in a correct-by-construction manner. K
specifications (which are ML theories) exist for a variety of languages
including C, Java, JavaScript, and EVM. The K prover~\cite{stefanescu:semantics}
supports coinductive proofs of reachability properties that underlie the
correctness of programs defined using K. It is based on Reachability Logic,
formalized in ML, and proofs can be desugared into ML proofs.

Challenges of encoding ML and proofs generated by the K prover in Dedukti
include the translation of the ML theory expressing the definition of a
particular language (where the ML axioms are patterns expressing transition
relations), the translation of the ML proof system, including its coinductive
reachability rules, and the certification of the automatic proof tools (such as
SMT solvers) that the prover relies upon. Our objective in this project is to
have translated into Dedukti the semantics of (at least) a simple programming
language such as IMP, the proof system for reachability properties and a few
proofs of paradigmatic program properties.
...
specifications are written in \textsf{Smart}, a functional language that
...
% \textsf{Smart} is based on a classical first-order logic with rank-1
% polymorphism, algebraic data types, recursive definitions, and inductive
% predicates.
% Proof obligations generated from \textsf{Smart} models are presented as virtual
...
of the \textsf{Smart} language, and the tool performs static analysis to ensure
...
about 400kloc of \textsf{Smart}, covering all process management including page
...
\textsf{Smart} models and proofs to Dedukti is that ProvenTools does not
manipulate formulas or proof terms explicitly. Instead, models are translated to
an intermediate format called \textsf{Smil} that resembles control flow graphs
...
benefit of encoding \textsf{Smart} in Dedukti will be to enable cross-checking
...
Matching Logic and Minlog have strong support for co-induction and co-recursion
...
in this respect close to Dedukti, although the details differ. Matching Logic
...
of these systems and libraries can differ greatly. For example,
...
those systems is an essential task: for example, simply combining
all the different theories without adaptations can create an
inconsistent and, therefore, useless combination.
%
...
assistants, and finally to build tools to construct these alignments. We
...
choice, into constructive proofs.  This transformation will not only be
...
In order to carry out this plan, we will study bidirectional encodings
...
Bishop style modulated Cauchy sequences, regular Cauchy sequences,
Dedekind cuts, and stream representations using co-recursion.
...
to allow not only exact matches, but also matches with respect to
