...
\emph{e.g.}\ the E prover for first-order logic, and it uses Zenon Modulo and
...
\item Some steps in the proof traces are not provable: their
  ...
  proof. This is the case for instance for the Skolemization step performed by an
  ...
  Skolemization and definition introduction, which are sufficient
...
\item Dedukti-producing provers used by Ekstrakto, namely Zenon Modulo
  ...
  \emph{e.g.}\ arithmetic or bit-vectors, as could be output by SMT
...
important areas of mathematics (\emph{e.g.}\ analysis and algebra) and
computer science (\emph{e.g.}\ compilers) that support applications in
...
(cf. \cite{brucker.ea:ontologies-certification:2019}).
...
then enables queries that span multiple libraries, \emph{e.g.}\ to find all
...
produce proof objects, and much background knowledge (\emph{e.g.}\ stemming
...
that are omitted in proofs (\emph{e.g.}\ trivial equalities such as $1+1=2$). In the
...
also align the many different mathematical concepts (\emph{e.g.}\ sets,
...
certain additional axioms, \emph{e.g.}\ the axiom of choice.
...
only in the treatment of certain corner cases (\emph{e.g.}\ Hilbert's axioms of
