...
\item[LIL 5:] A tool has been built to analyze the Dedukti proofs
...
we have preliminary results and that we plan to bring to a high
level of integration. Those in the second half (Minlog, PVS, 
Mizar, TLA$^+$, Why3, and LFSC) with which we are
starting and for which our goal are generally less ambitious.
...
algorithms or access control policies have been formalized
...
gain confidence in their specifications, and it can be used as a
...
\item A few of the systems rely on AC rewriting (i.e.~rewriting modulo
  ...
  infeasible. Thus, expressing these theories requires improvements in
  Dedukti itself.
\end{itemize}
