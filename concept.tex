...
Finally, after discussing the concepts related to the networking
activities needed to collect the content of Logipedia and the
development of the infrastructure needed to make it accessible, let us
turn to joint research projects.
...
Dedukti.  There are other systems, that are still at level 0, that is,
we do not yet know how to express them: Mizar, TLA$^+$, PVS, Minlog,
Why3, and Smart, to which we must add Homotopy type theory, which is a
theory, but not a system.  These systems range from specialized
systems such as Minlog used in academia to mainstream systems such as
PVS or TLA$^+$ that have been used in major scientific or industrial
projects. They provide specific features that are relevant to their
target domains and are therefore preferred by users for developing
theories and proofs in these areas.  The objective is to bring them
(at least) to LIL $2$ in order to prepare the grounds for their
integration into Logipedia in the longer term.
...
will be able to cross-verify them.  Neither goal will be fully
achieved at the end of the project since higher Logipedia integration
levels are necessary for exporting or importing full
libraries. Nevertheless, the work carried out here is a necessary
stepping stone and enabler for making Logipedia a universally accepted
