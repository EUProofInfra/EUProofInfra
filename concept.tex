...
The multiplication of systems and theories jeopardises the
...
logical framework will permit, in a similar way, to analyse which
...
need to analyse this proof in order to determine which symbols, axioms
...
of the Four Colour theorem and the Feit-Thompson theorem. In 2013, Coq
...
\begin{wrapfigure}{r}{30mm}\centering\logo[20mm]{Matita}\end{wrapfigure}
...
to \begin{wrapfigure}{r}{30mm}\centering\logo{Agda}\end{wrapfigure} Coq and
...
\noindent
\begin{wrapfigure}{r}{30mm}\centering\logo{AtelierB}\\[3mm]
  \logo[9mm]{Rodin}\end{wrapfigure}
...
formalised respectively in the Event B and classical B methods.  The
...
\item Many Agda libraries (as well as some Coq libraries) heavily rely
  ...
  type checking. This blow-up has to be tamed by the choice of a
...
\item A few of the systems rely on AC rewriting (that is, rewriting modulo
...
transient information by either logging or re-synthesising it on
...
\item{\bf Producing more compact proofs.}  When translating a proof
  ...
  normalising it or by adding type annotations.  The translation of
...
normalisation or duplication of (parts of) proofs. We will also
...
predicate logic output proofs in a standardised language~\cite{DBLP:conf/csr/Sutcliffe07},
...
confidence that the approach will be successful, it is work in progress. In
...
\item Up to now, Ekstrakto can only understand traces in TSTP format as
  ...
  from SMT solvers, as well as all formats that will appear when
  instrumenting other automatic theorem provers.
...
\item Some steps in the proof traces are not provable: their
  ...
  proof. This is the case for instance for the Skolemisation step performed by an
  automated theorem prover for predicate logic, for the introduction of
  new definitions, as well as for the RAT property in traces produced by
  ...
  Skolemisation and definition introduction, which are sufficient
  for managing all traces from the
...
\item Dedukti-producing provers used by Ekstrakto, namely Zenon Modulo
  and ArchSAT, are meant for pure predicate logic. However, it will be necessary  to deal with proof traces that use some specialised theory,
  for example arithmetic or bit-vectors, as could be output by SMT
  ...
  on said theory. These provers need not be very optimised, since
  trace steps are relatively small; this should help to produce Dedukti
  traces. A way to achieve this could be to extend Zenon Modulo:
  indeed, Zenon modulo can find proofs modulo arithmetic, but it cannot
  produce a Dedukti proof yet.
\end{compactitem}
\end{enumerate}
...
before~\cite{Irving-deepmath}.  We will experiment with integrating techniques
...
algorithms to use, they display blind spots, i.e., specific cases that are not solved
...
allows for much more efficient formalisation~\cite{Hales-Developments}.
...
%
The number and complexity of C programmes verified with {\em a posteriori}
...
\begin{wrapfigure}{r}{30mm}\centering\logo{AFP}\end{wrapfigure}
In Feb-2020, the
Archive of formal proofs consisted of more than 500 entries (articles of formalised
...
of the HOL Light system but has generalised it from real numbers to
...
to be modularised. This requires a significant refactoring effort.
%
...
particular the ability to describe ontologies.  This will notably entail annotating those statements in the library that correspond
...
transform). A formalisation of basic properties of the Laplace transform is already available in the Archive of formal proofs.
...
one is called the arithmetisation and coordinatisation of geometry and
it represents the culminating result of both Hilbert\cite{Hilbert}
and Tarski\cite{Tarski}.
As of now, there is no other formalisation of this result
...
formalisation enables to obtain automatic proofs based on geometric
...
arithmetisation and coordinatisation of geometry are available for
...
formalised and proved largely in HOL
Light. HOL Light
has a large and rich analysis and geometry library, with some results
not formalised in any other system. This motivates the project of
...
\begin{wrapfigure}{r}{45mm}\centering\logo[4cm]{CakeML}\end{wrapfigure}This is a cutting edge compiler library and one of only two
...
at the centre of the networking activities that will populate
...
source code of the Logipedia website itself as well as the formal proof libraries.
\item
A
...
available, released under some free licence, and accessible on the
...
is an example of a highly customisable and scalable search engine for
...
Mathematical Library (MML): terms, which are (lemmatised or stemmed)
...
\paragraph*{Interface for teaching.}
...
different from what the programme had pre-computed using automated
...
\item Proofs must be displayed in a structured and readable way. In
  ...
  typography equivalent to current standards (Latex, Mathjax,\dots)
...
validate formal and semi-formal information.
...
Annotation with meta-data is particularly relevant in connection with the concept alignments
...
These systems range from specialised systems such as Minlog
...
rich library of formalised mathematics would be beneficial to
...
The Mizar community maintains a centralised database, the Mizar Mathematical
...
mathematical formalisations, Mizar has been also used for developing formal
...
analysing and verifying \tlaplus specifications are the model checker
...
extracting programmes from constructive proofs, and has also been extended for extracting
...
realises the statement, viewed as a specification.
...
co-recursion. We plan to support programme extraction to Dedukti through
realisability, and also aim to be able to import a subset of Dedukti
...
for designing programmes and proofs. Programmes and specifications are
written in Smart, a functional language that emphasises a separation
...
The logic underlying ProvenTools is similar to that of other programme
...
with sink states representing erroneous behaviour. Proof obligations are
...
\paragraph*{\href{https://github.com/kframework/k}{K Prover}}
...
operational behaviour and formal properties of a programme.
 \end{minipage}
&
\begin{minipage}{3cm}
  \logo{K}
...
reachability properties that underlie the correctness of programmes
defined using K. It is based on Reachability Logic, formalised in
Matching Logic.
...
paradigmatic programme properties.
...
proof search, and PVS emphasises predicate sub-typing, which is also
...
corpus of mechanised mathematical theories. As these libraries span a
...
archive a successful interoperability of both proof assistants and
formalised mathematical libraries. This starts with identifying the
correspondences between the concepts underpinning these
formalisations, that is their alignment. Even when the various proof
...
combination.
%
Alignments can also be useful for joining formalisations done within a
...
representations using co-recursion, all such different formalisations
...
of the Logipedia project to the virtualisation of computer operating
systems. In such virtualised systems, it is not sufficient for the
...
different definitions of these concepts.  For virtualisation to
...
formalised mathematics, we will pay special attention to their
...
aligning different existing formalisations of geometry.
...
different axiomatisations. In practice, we want to use a theorem
...
\paragraph*{Proof engineering services.}
...
facilitate access to the available body of formalised mathematical and
...
\item A very precise definition of alignments should be given and a
  format (and an ontology) for describing and organising alignments
  must be described.
...
\subsubsection*{Other projects (not directly funded by the European Union)}
...
formalisation of the main results (constructions, definitions, proofs,
...
clear as Formal Abstracts provide a first step towards formalising all
the mathematical knowledge. The two projects are very complementary as 
Formal abstracts offers to formalise everything except proofs and
Logipedia to formalise proofs.
\\
\hline
{\bf ProofPeer}
&
...
objects: programmes and proofs, but they share the same values and the
...
The participation of Aaron Stump to the advisory board is a factor facilitating the synergy.
\\
\hline  
{\bf Xena} 
&
