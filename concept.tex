...
The multiplication of systems and theories in which
formal proofs are developed jeopardizes the universality of logical
truth. But it is not the first time in history this has happened.
For instance, in the 19$^{\mbox{\footnotesize th}}$ century,
...
allowed mathematicians to understand which axiom was used in which proof, restoring
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\cmsection{The Logipedia integration levels}

To make a formal proof, developed in some system $X$, accessible
...
associated proof library in Logipedia, we introduce a metric: {\em the
  six Logipedia integration levels}.
...
These systems can be roughly divided into two groups. For those in the
first half of the table (Matita, HOL Light, FoCaLize, Coq, Agda,
Atelier B, ProB, Isabelle/HOL, HOL4, TSTP, Smart, and Rodin)
we have preliminary results and we plan to bring them to a high
level of integration. For those in the second half (Minlog, PVS, 
Mizar, TLA$^+$, Why3, and LFSC) we are only starting out
and our goals within this project are generally less ambitious.
...
Very large proofs, requiring large libraries of auxiliary results, have been developed in
some proof systems. These large proofs and libraries are
interesting for our project in two respects. First, they constitute an
excellent benchmark for the methods developed above. Secondly, they will
serve as the source for the lion's share of proofs in Logipedia for end-user
applications.
...
needed to collect the content of Logipedia and the development of the infrastructure
...
There are other systems, that are still at level 0, that is, we
...
LIL $2$ in order to prepare the grounds for their integration
...
Minlog has strong support for co-induction and co-recursion
