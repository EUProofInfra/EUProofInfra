...
proofs in {\textsf Coq}, in {\textsf Matita}, in {\textsf HOL Light}, in {\textsf
...
\item indexing mechanisms for mathematical formulas, and search engines
  ...
  \item a structure of mathematical knowledge in theories, books,
    ...
    be inherited from the libraries imported in {\textsf Logipedia}, from
...
Proof systems are research infrastructure. {\textsf Logipedia} is not yet
...
\item As mathematics and software are in any part of modern science,
  {\textsf Logipedia} will also have an impact on other sciences. In
...
between relatively close communities, such as the {\textsf Coq} and {\textsf
...
We have already organized two {\textsf Logipedia} meetings that have
...
as contributors or as members of the future {\textsf Logipedia}
...
A side effect of the construction of the {\textsf Logipedia} platform is
to incent all the proof systems to be able to produce proofs in a
common language. Such proofs can then be all be checked by {\textsf
...
proofs developed in different systems can be combined in {\textsf
Logipedia} itself.
\end{enumerate}
...
Then, some compagnies, specially some related to the {\textsf B} community
and the railway industry, are part full partners of the project and
will contribute to the production of {\textsf Logipedia} proofs from
{\textsf B} provers. 

{\color{red} Club of industrial users (Frédéric, can 
you explain the impact here?)}

{\color{red} Impact for the {\textsf B} community and the railway industry
  (Yamine?)}
...
- Each import feature to {\textsf Logipedia} and each export
feature from {\textsf Logipedia} is a 
Key Exploitable Result.
