...
theory. It is critically important that these different logical
foundations can be related to each other in ways that will ensure
their interoperability. Even when one has successfully aligned these
different logical foundations, one must also align the many different
mathematical concepts and theorems that are in the libraries produced
by the various proof assistants. Thus, successful interoperability of
...
these formalizations, i.e., their alignment.
...
Even when the theories of different proof-assistants are all
formulated within Dedukti, In order to combine the power of different
proof assistants and their respective communities, it is necessary to
bring together libraries from different proof assistants.  Even if
these different libraries are all formulated within Dedukti, the
proper alignment of the theories underpinning those systems is an
essential task since simply taking the union of all the different
theories can create an inconsistent and, therefore, useless
combination.  Alignments can also be useful for joining formalizations
done within a single prover, since different formal libraries
developed within a single system can introduce different definitions
of the same mathematical concept (each more suitable to the library
...
\begin{itemize}
\item alignments at the levels of logic deals with aligning logical foundations,
\item alignments of theorem proving objects deals with aligning types, constants and
  theorems,
\item alignment of proofs deals with aligning tactics and
  inference rules that have the same effect on the proof state.
\end{itemize}

Within this work package, we will primarily focus on the alignment of
logics and theorem proving objects.
...
articulate our work according to the following key parameters to
distinguish between theories: classical vs intuituitionistic logic,
predicative vs impredicative definitions, treatments of equality, and
presence of additional axioms, e.g.~axiom of choice.
...
fragment of first-order logic. This will be of interest because we
believe that, over such a theory, it is possible to transform
classical proofs, using the law of excluded middle and the axiom of
choice, into constructive ones. This will not only be a theoretical
result, as we seek to obtain practical and feasible algorithms to
transform proofs obtained in one system into proofs of another
system. This adds a new dimension to a long-standing development of
proof theory, still very much unexplored.
...
will be in contrast to the standard reverse mathematics developed in
the context of second-order arithmetic. Again, the presence of proof
terms adds a new dimension to a well-established topic.

One upshot of this work is to make features of one system available to
others. For example, Minlog has a feature of the refined
Dragalin-Friedman $A$-translation, so that classical proofs in a
certain class can be constructivised and ready to carry out program
extraction.  We would like to make this feature of Minlog available to
other proof assistants through Dedukti.  In order to carry it out, we
study both-directed encodings between a subsystem of Dedukti and
Minlog.
...
into aligning different existing formalizations of geometry.

There are several ways to define the numbers. For the natural numbers,
one can for example choose a definition based on Peano’s axioms which
rely on an unary representation of numbers which is very suitable for
proving theorems. But for computing, a definition based on a binary
representation is better. For real numbers and functions, the
definitions differ also between the different libraries, as there are
the Bishop style modulated Cauchy sequence, regular Cauchy sequence,
Dedekind cut, and also stream representations due to corecursion. For
geometry, it can be defined using Hilbert’s, Euclid’s or Tarski’s
axioms using a synthetic approach or using coordinates and
algebra. The first step is hence to import inside Dedukti the
...
Although alignments could be manually discovered and described, due to
the sheer size of existing formal libraries, best results could be
obtained only if automated support for finding and organizing
...
derivations. In order to enable flexible alignments, we set to allow
not only exact matchings, but also matchings with respect to a given
similarity measure, as in \cite{???}.

An alternative to manually specifying an ontology for alignments, is
to use unsupervised learning methods to find them between proof
...
is stated and proved in the logic of $L’$, it will talk about a
...
its simplest form, this simply a search interface, where the user
enters $t$ and the system also find $t’$ because it uses the alignment
$(c,c’)$. Secondly, we will develop translation services that use
alignments to port proofs across libraries. This will allow porting
the proof of $t’$ in $L’$ to a (potential) proof of $t$ in $L$.
...
\begin{itemize}
\item A very precise definition of aligments should be given and a
...
\item A case study of geometry must show that it is possible to align
  ...
  on different aproaches (synthetic vs analytic).
\end{itemize}
